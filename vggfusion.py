...
def get_activation(model, layer_numbers, input_image, EigVal, EigVec):
    outs = []
    feats = []
    feats.append(EigVal)
    out = input_image
...
def fuse(vis, ir, model=None):
    ...
    EigVal, EigVec = calculateEig(A)
    
    relus_vis = get_activation(model, relus, vis_in, EigVal, EigVec)
    relus_ir = get_activation(model, relus, ir_in, EigVal, EigVec)
